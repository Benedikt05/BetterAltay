 ...
 * @internal
 */
final class RuntimeBlockMapping776{
	...
	public static function init() : void{
		$canonicalBlockStatesFile = file_get_contents(RESOURCE_PATH . "vanilla/canonical_block_states776.nbt");
		if($canonicalBlockStatesFile === false){
			throw new AssumptionFailedError("Missing required resource file");
		}
		$stream = new NetworkBinaryStream($canonicalBlockStatesFile);
		$stream->setProtocol(ProtocolInfo::PROTOCOL_1_21_60);
		$list = [];
		while(!$stream->feof()){
	...
	private static function setupLegacyMappings() : void{
		...
		$legacyStateMapReader->setProtocol(ProtocolInfo::PROTOCOL_1_21_60);
