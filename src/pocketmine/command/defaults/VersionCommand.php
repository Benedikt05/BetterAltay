...
class VersionCommand extends VanillaCommand{
	...
	public function execute(CommandSender $sender, string $commandLabel, array $args){
		...
		if(count($args) === 0){
			$sender->sendMessage("This server is running " . $sender->getServer()->getName() . " " . $sender->getServer()->getBetterAltayVersion() . " based on PocketMine-MP " . $sender->getServer()->getPocketMineVersion() . " for Minecraft: Bedrock Edition " . $sender->getServer()->getVersion() . " (protocol versions " . implode(", ", ACCEPTED_PROTOCOL) . ")");
